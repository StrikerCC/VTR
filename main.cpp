...
//    auto voxel_grid_c_arm = open3d::geometry::VoxelGrid::CreateFromTriangleMesh(*pc_c_arm, voxel_size);
    ...
    c_arm_poses_motion_planning.emplace_back(Eigen::Matrix4d({{0, 0, 1, 100}, {1, 0, 0, -400}, {0, 1, 0, 850}, {0, 0, 0, 1}}));
    for (const auto &tf_motion_plan : ReadTfFromFile("")) {
        c_arm_poses_motion_planning.push_back(tf_motion_plan * c_arm_poses_motion_planning.at(0));
    }
...
//    c_arm_poses_motion_planning.push_back(std::make_shared<Eigen::Transform<double, 3, 0x1>>(Eigen::Matrix4d({{0, 0, 1, 600}, {1, 0, 0, -400}, {0, 1, 0, 150}, {0, 0, 0, 1}})));
    ...
    for (int i_iter = 0; i_iter < nLoop; i_iter++) {
        ...
        for (int i_c_arm_pose = 0; i_c_arm_pose < c_arm_poses_motion_planning.size(); i_c_arm_pose++) {     // rendering voxel as a_arm motion planned
            ...
            // vis debug
            std::cout << indent << "pose " << i_c_arm_pose << ": " << c_arm_poses_motion_planning.at(i_c_arm_pose).matrix() << std::endl;
            std::cout << indent << "pose " << i_c_arm_pose << " vtr # of collision: " << num_of_overlapping << std::endl;
            if (flag_vis_debug) {
...
int HasOverlapping(open3d::geometry::VoxelGrid& voxel_grid_tgt, const open3d::geometry::PointCloud& pc_src, open3d::geometry::VoxelGrid& voxel_grid_tgt_overlapping) {
    ...
    voxel_grid_tgt_overlapping.Clear();     // overlapping voxel grid
